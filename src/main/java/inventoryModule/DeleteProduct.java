		...
		try (PrintWriter out = response.getWriter()) {
			...
			while (rs.next()) {
				path = getServletContext().getRealPath(File.separator + "img" + File.separator + "shop" + File.separator
						+ "products" + File.separator + product_id + File.separator + rs.getString("product_item_id"));
			...
			path = getServletContext().getRealPath(File.separator + "img" + File.separator + "shop" + File.separator
					+ "products" + File.separator + product_id);
			File dir = new File(path);
			dir.delete();
			ps = cn.prepareStatement("DELETE FROM product WHERE product_id=?");
			ps.setString(1, product_id);
			ps.execute();
			out.print("1");
