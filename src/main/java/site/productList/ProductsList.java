...
public class ProductsList extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter out = response.getWriter()) {
			...
			int LIMIT = SiteConstants.PRODUCTS_RS_LIST_SIZE;
			int page;
			if (p_no != null) {
			...
			for (Map.Entry<String, Integer> item : productItems.entrySet()) {
				...
				// Product
				out.print("<div class='card product-card product-list'>");
				...
				out.print("<div class='card-body py-2'><a class='product-meta d-block fs-xs pb-1' href='#'>"
						+ rs.getString("brand_name") + "</a>");
				out.print("<h3 class='product-title fs-base'><a href='product-view.jsp?pid="
						+ rs.getString("product_id") + "&piid=" + rs.getString("product_item_id") + "'>"
				...
				if (layoutOption.equals("1")) {
					...
					for (Map.Entry<String, Integer> color : colors.entrySet()) {
						...
						out.print("<input class='form-check-input chcolor' data-pid=" + rs.getString("product_id")
								+ " data-piid=" + color.getValue() + " type='radio' name='" + nameVarient + "' value='"
								+ color.getKey() + "' id='" + nameVarient + color.getKey() + "' ");
					...
					out.print("<div class='d-flex mb-2'><select class='form-select form-select-sm me-2' id='"
							+ nameSelect + "' onchange='changeImage(this.value," + rs.getString("product_id")
							+ "	name='" + nameSelect + "' style='max-width:6rem;'>");
					...
					out.print("</div><div class='text-start'><a class='nav-link-style fs-ms quick' data-pid="
					...
					// Add To Cart
					if (SiteConstants.ADD_TO_CART_ON_GRID_LIST) {
						out.print("<button class='btn btn-primary btn-sm mb-2 add-to-cart' data-pid="
								+ rs.getString("product_id")
								+ " type='button'><i class='ci-cart fs-sm me-1'></i>Add to Cart</button>");
					}

					out.print("<div class='text-start'><a class='nav-link-style fs-ms quick' data-pid="
							+ rs.getString("product_id") + " data-piid=" + rs.getString("product_item_id")
							+ " href='#quick-view' data-bs-toggle='modal'><i class='ci-eye align-middle me-1'></i>Quick view</a></div>");
					...
					for (Map.Entry<String, Integer> size : CapSize.entrySet()) {
						...
						out.print(
								"<div class='form-check form-option form-check-inline mb-2'><input class='form-check-input' type='radio' name='"
										+ nameVarient + "' id='" + nameVarient + size.getKey() + "' value='"
										+ size.getKey() + "' ");
						for (Map.Entry<String, String> variation : variations.entrySet()) {
							if (size.getKey().equals(variation.getValue())) {
								out.print(" checked");
				...
				out.print("</div></div></div></div><div class='border-top pt-3 mt-3'></div>");
			...
			// Pagination
			ps = cn.prepareStatement("SELECT COUNT(product_id) FROM product WHERE category_id=?");
			ps.setString(1, category_id);
			rs = ps.executeQuery();
			...
			if (page != TOTAL_PAGES) {
				out.print("<ul class='pagination' id='nextpageList'>");
